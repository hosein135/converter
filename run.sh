#!/usr/bin/env bash
# =============================================================================
# run.sh — Auto setup-or-start for the AV2 / xHE-AAC converter (Linux / macOS)
#
# Host bootstrap: curl (static if missing) + Nix (official installer).
# Python GUI, AVM (avmenc), exhale, dav2d-av2, vlc-av2, and ffmpeg come from
# devops/flake.nix (nixpkgs 25.05).
#
# Usage:
#   ./run.sh                # Nix env + GUI
#   ./run.sh --force-setup
#   ./run.sh --prep-only    # fetch/build tools, do not open the GUI
#   ./run.sh --convert FILE
#   ./run.sh --help
# =============================================================================
set -euo pipefail
...
info()  { echo -e "${GREEN}[av2-converter]${NC}  $*"; }
warn()  { echo -e "${YELLOW}[av2-converter]${NC}  $*"; }
error() { echo -e "${RED}[av2-converter]${NC} $*" >&2; }
step()  { echo -e "${CYAN}[av2-converter]${NC}  $*"; }

# WSL/Windows mounts and some editors leave CRLF in shell scripts.
sanitize_shell_file() {
    local f="$1"
    [ -f "${f}" ] || return 0
...
cd "${SCRIPT_DIR}"
READY_MARKER="${SCRIPT_DIR}/.av2converter-nix-ready"
BOOTSTRAP_DIR="${SCRIPT_DIR}/.av2converter-bootstrap"
BOOTSTRAP_BIN="${BOOTSTRAP_DIR}/bin"
DATA_DIR_DEFAULT="${SCRIPT_DIR}/.av2converter-data"
FLAKE_DIR="${SCRIPT_DIR}/devops"
CURL_STATIC_VERSION="8.20.0"
NIX_INSTALL_URL="https://releases.nixos.org/nix/nix-2.24.12/install"
SYSTEM_CACHE_ROOT="${XDG_CACHE_HOME:-${HOME}/.cache}/av2-converter"
SYSTEM_CACHE=""
SYSTEM_PROFILE=""
SYSTEM_DEVENV=""
SYSTEM_READY=""
...
for arg in "$@"; do
    case "$arg" in
        --help|-h)
            sed -n '3,16p' "$0" | sed 's/^# //'
            exit 0 ;;
        --__launch)    INNER_LAUNCH=true ;;
        --force-setup) FORCE_SETUP=true ;;
        --prep-only)   PREP_ONLY=true ;;
        --convert)
            CONVERT_FILE="__next__" ;;
        *)
            if [ "${CONVERT_FILE}" = "__next__" ]; then
                CONVERT_FILE="$arg"
            else
                FORWARD_ARGS+=("$arg")
            fi
...
if [ "${CONVERT_FILE}" = "__next__" ]; then
    error "--convert requires a file path"
    exit 1
fi
...
hash_nix_inputs() {
    local files=()
    files+=("${FLAKE_DIR}/flake.nix")
    [ -f "${FLAKE_DIR}/shell.nix" ] && files+=("${FLAKE_DIR}/shell.nix")
    if [ -d "${FLAKE_DIR}/pkgs" ]; then
        # shellcheck disable=SC2046
        files+=($(find "${FLAKE_DIR}/pkgs" -name '*.nix' | sort))
    fi
    if command -v sha256sum >/dev/null 2>&1; then
...
ensure_flake_lock() {
    ...
    if [ ! -f "${FLAKE_DIR}/flake.lock" ]; then
        step "Creating flake.lock (nixpkgs 25.05 + AV2 tool sources) ..."
...
realize_nix_shell() {
    ...
    info "First run builds AVM, dav2d-av2, exhale, and vlc-av2 from source — this can take a long time."
...
setup_first_time() {
    step "First-time (or incomplete) setup — preparing AV2 converter Nix environment ..."
    ensure_curl
    ensure_nix
    ensure_flakes
    ensure_flake_lock
...
export_runtime_env() {
    export AV2CONVERTER_ROOT="${SCRIPT_DIR}"
    export AV2CONVERTER_DATA_DIR="${AV2CONVERTER_DATA_DIR:-${DATA_DIR_DEFAULT}}"
    mkdir -p "${AV2CONVERTER_DATA_DIR}/converted"
...
launch_app() {
    ...
    if [ "${PREP_ONLY}" = true ]; then
        info "Prep-only — Nix env is ready (avmenc, exhale, vlc-av2)."
        command -v avmenc >/dev/null && info "avmenc: $(command -v avmenc)"
        command -v exhale >/dev/null && info "exhale: $(command -v exhale)"
        command -v vlc-av2 >/dev/null && info "vlc-av2: $(command -v vlc-av2)"
        return 0
    fi
    if [ -n "${CONVERT_FILE}" ]; then
        step "Converting ${CONVERT_FILE} ..."
        exec python -m av2converter --convert "${CONVERT_FILE}" "${FORWARD_ARGS[@]+"${FORWARD_ARGS[@]}"}"
    fi
    step "Starting AV2 Converter GUI ..."
    exec python -m av2converter "${FORWARD_ARGS[@]+"${FORWARD_ARGS[@]}"}"
}

run_inside_nix() {
    step "Nix environment ready — starting AV2 converter ..."
    local extra=()
    if [ "${PREP_ONLY}" = true ]; then
        extra+=(--prep-only)
    fi
    if [ -n "${CONVERT_FILE}" ]; then
        extra+=(--convert "${CONVERT_FILE}")
    fi
    extra+=("${FORWARD_ARGS[@]+"${FORWARD_ARGS[@]}"}")
    ...
    if [ -f "${SYSTEM_DEVENV}" ]; then
        ...
        nix_cmd develop "${FLAKE_DIR}" \
            ...
            --offline \
            --no-update-lock-file \
            --command bash "${SCRIPT_DIR}/run.sh" --__launch "${extra[@]}"
    fi
}

main() {
    if [ "${INNER_LAUNCH}" = true ]; then
        launch_app
        exit 0
    fi
    ...
    if [ "${FORCE_SETUP}" = true ]; then
        ...
        info "AV2 converter Nix environment already cached — starting."
        source_nix_profile || true
        enable_flakes
        restore_cached_flake_lock
    else
